...
import { ETAPAS_PIPELINE, usePipelineActions } from "@/components/clientes/pipeline-actions";
...
const ETAPA_ICONS: Partial<Record<EstagioLead, React.ElementType>> = {
  NOVO_LEAD: PhoneCall,
  CONTATO_INICIAL: MessageCircle,
  PRIMEIRO_ORCAMENTO: FileText,
  QUALIFICACAO: ClipboardList,
...
function PipelineTracker({
  ...
  const ctx = { leadId, clienteId, clienteNome, estagioAtual: estagio, numeroOrcamentoAtual };
  const { abrirEtapa, abrirConfirmar, abrirCancelar, isPending, dialogs } = usePipelineActions({ onUpdate });

  const isFechado = estagio === "FECHADO_GANHO" || estagio === "FECHADO_PERDIDO";
  const currentIdx = ETAPAS_PIPELINE.findIndex(e => e.estagio === estagio);
      ...
      <div className="bg-card border rounded-xl p-4">
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {ETAPAS_PIPELINE.map((etapa, idx) => {
            const Icon = ETAPA_ICONS[etapa.estagio]!;
            ...
            return (
              <div key={etapa.estagio} className="flex items-center gap-1">
                <button
                  type="button"
                  disabled={isPending}
                  onClick={() => abrirEtapa(ctx, etapa.estagio)}
                  className={cn(
