...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    await WhatsAppService.desconectarSessao(params.id, payload);
    return NextResponse.json({ ok: true });
  } catch (error) {
    if (error instanceof PosseError) return NextResponse.json({ error: error.message }, { status: 403 });
    console.error("[WA Desconectar]", error);
    return NextResponse.json({ error: "Erro ao desconectar sessão" }, { status: 502 });
  }
}
