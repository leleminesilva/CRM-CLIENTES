...
- **Fase 5** — provisionamento real da VPS/Docker/Evolution. Roteiro passo a passo em [`docs/deploy/whatsapp-vps-setup.md`](../deploy/whatsapp-vps-setup.md). **Em andamento (2026-08-31):** drift de schema que motivava o standby foi confirmado resolvido (`prisma migrate diff` contra produção e local dá vazio); `WHATSAPP_STANDBY` deixou de ser `const` hardcoded e passou a ler `NEXT_PUBLIC_WHATSAPP_STANDBY` (ausência = standby ligado, então produção segue congelada até a env ser setada na Vercel); roteiro corrigido pros nomes de env var da Evolution v2 e pra um primeiro teste local via túnel ngrok. Falta: subir a VPS, verificar `EvolutionProvider` contra a instância real (paths/eventos/base64 de mídia) e o teste ponta a ponta.
