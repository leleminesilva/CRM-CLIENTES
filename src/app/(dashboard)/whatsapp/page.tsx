...
interface AcaoAuto {
  ...
  texto?: string;
  textos?: string[];
  textosPorSessao?: Record<string, string[]>;
  etapa?: EtapaQuadro;
  status?: ConversaStatus;
  etiqueta?: string;
}
...
function CardAutomacao({
  a, podeEditar, etapaNome, viewSessaoId, onToggle, onEditar, onExcluir,
}: {
  a: Automacao;
  podeEditar: boolean;
  etapaNome: Record<string, string>;
  viewSessaoId?: string | null;
  onToggle: () => void;
  onEditar: () => void;
  onExcluir: () => void;
}) {
          ...
          {a.acoes.map((ac, i) => (
            <span key={i} className="inline-flex items-center gap-1">
              ...
              <span className="inline-flex items-center gap-1 rounded-md border border-green-500/35 bg-muted/60 px-2 py-0.5 text-xs font-semibold text-green-600 dark:text-green-400">
                ...
                {ac.tipo === "ENVIAR_MENSAGEM" && viewSessaoId && ac.textosPorSessao?.[viewSessaoId]?.length
                  ? " · personalizada"
                  : ""}
              </span>
            </span>
...
function FormAutomacao({
  inicial, etapas, viewSessaoId, viewSessaoNome, onClose, onSaved,
}: {
  inicial: Automacao | null;
  etapas: EtapaCol[];
  // Canal selecionado no rail à esquerda no momento — cada atendente edita a
  // PRÓPRIA mensagem de "Enviar mensagem" vendo o canal dele selecionado, sem
  // mexer na de ninguém. Não confundir com o "sessaoId" abaixo, que é o canal
  // de ENVIO da regra (pra quem ela manda), não de quem está editando.
  viewSessaoId?: string | null;
  viewSessaoNome?: string;
  onClose: () => void;
  onSaved: () => void;
}) {
  ...
  const [salvando, setSalvando] = useState(false);
  // Por padrão edita a mensagem do canal selecionado (viewSessaoId). O
  // atendente pode alternar pra ver/editar a mensagem padrão (a que quem
  // ainda não personalizou a própria recebe).
  const [editarPadrao, setEditarPadrao] = useState(!viewSessaoId);
  const personalizando = !!viewSessaoId && !editarPadrao;

  const listaDaAcao = (a: AcaoAuto): string[] => {
    if (personalizando) {
      const propria = viewSessaoId ? a.textosPorSessao?.[viewSessaoId] : undefined;
      if (propria?.length) return propria;
      return a.textos?.length ? a.textos : [""];
    }
    return a.textos?.length ? a.textos : [""];
  };
  const setListaDaAcao = (i: number, arr: string[]) => {
    setAcoes((as) =>
      as.map((a, j) => {
        if (j !== i) return a;
        if (personalizando && viewSessaoId) {
          return { ...a, textosPorSessao: { ...(a.textosPorSessao ?? {}), [viewSessaoId]: arr } };
        }
        return { ...a, textos: arr };
      }),
    );
  };
  const temPersonalizacao = (a: AcaoAuto) => !!viewSessaoId && !!a.textosPorSessao?.[viewSessaoId]?.length;
  const removerPersonalizacao = (i: number) => {
    if (!viewSessaoId) return;
    setAcoes((as) =>
      as.map((a, j) => {
        if (j !== i || !a.textosPorSessao) return a;
        const resto = { ...a.textosPorSessao };
        delete resto[viewSessaoId];
        return { ...a, textosPorSessao: resto };
      }),
    );
  };
  ...
  async function salvar() {
    ...
    // Normaliza ações: ENVIAR_MENSAGEM leva "textos" (padrão/compartilhada) e
    // "textosPorSessao" (a personalizada de cada canal), ambos sem vazios.
    const limpar = (arr?: string[]) => (arr ?? []).map((t) => t.trim()).filter(Boolean);
    const acoesOut = acoes.map((a) => {
      if (a.tipo !== "ENVIAR_MENSAGEM") return a;
      const textosPorSessao: Record<string, string[]> = {};
      for (const [sid, lista] of Object.entries(a.textosPorSessao ?? {})) {
        const limpa = limpar(lista);
        if (limpa.length > 0) textosPorSessao[sid] = limpa;
      }
      return {
        tipo: a.tipo,
        textos: limpar(a.textos),
            ...
            {acoes.map((a, i) => (
              <div key={i} className="rounded-lg border border-border p-2.5 space-y-2">
                ...
                {a.tipo === "ENVIAR_MENSAGEM" && (
                  <>
                    {viewSessaoId && (
                      <div className="flex items-center justify-between text-[11px] bg-muted/40 rounded-md px-2 py-1.5">
                        <span className="text-muted-foreground">
                          {personalizando ? (
                            <>Mensagem de <b className="text-foreground">{viewSessaoNome}</b></>
                          ) : (
                            <>Mensagem <b className="text-foreground">padrão</b> (quem não personalizar recebe essa)</>
                          )}
                        </span>
                        <div className="flex items-center gap-2">
                          {personalizando && temPersonalizacao(a) && (
                            <button onClick={() => removerPersonalizacao(i)} className="text-muted-foreground hover:text-destructive font-semibold">
                              Restaurar padrão
                            </button>
                          )}
                          <button onClick={() => setEditarPadrao((v) => !v)} className="text-blue-600 dark:text-blue-400 hover:underline font-semibold">
                            {personalizando ? "Editar a padrão" : `Editar a de ${viewSessaoNome ?? "este canal"}`}
                          </button>
                        </div>
                      </div>
                    )}
                    {listaDaAcao(a).map((t, vi) => (
                      <div key={vi} className="flex items-start gap-1.5">
                        <textarea
                          value={t}
                          onChange={(e) => {
                            const arr = [...listaDaAcao(a)];
                            arr[vi] = e.target.value;
                            setListaDaAcao(i, arr);
                          }}
                          rows={2}
                        ...
                        />
                        {listaDaAcao(a).length > 1 && (
                          <button
                            onClick={() => setListaDaAcao(i, listaDaAcao(a).filter((_, j) => j !== vi))}
                    ...
                    <div className="flex items-center justify-between">
                      <button
                        onClick={() => setListaDaAcao(i, [...listaDaAcao(a), ""])}
...
function PainelAutomacoes({ sessaoId, sessoes }: { sessaoId: string | null; sessoes: Sessao[] }) {
  const queryClient = useQueryClient();
    ...
    <div className="flex-1 min-w-0 overflow-y-auto">
      <div className="max-w-3xl mx-auto px-6 py-7 pb-16">
        ...
        {sessaoAtual && (
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-border bg-muted/40 px-3 py-1 text-xs text-muted-foreground">
            ...
            Editando mensagens de <b className="text-foreground">{sessaoAtual.nome}</b>
            <span className="text-[11px]">— troque o canal no menu à esquerda pra editar outra</span>
          </div>
        )}
      ...
      {form && (
        <FormAutomacao
          ...
          etapas={etapas}
          viewSessaoId={sessaoId}
          viewSessaoNome={sessaoAtual?.nome}
      ...
      <div className="flex-1 flex min-h-0 min-w-0 overflow-hidden">
        ...
        <PainelSessoes
          ...
          onSelect={(id) => {
            setSessaoId(id);
            setConversa(null);
            // Trocar de canal na Automações/Quadro só muda o contexto (de
            // quem eu tô editando/vendo); não precisa me tirar da aba.
            if (aba !== "automacoes" && aba !== "quadro") setAba("conversas");
          }}
        ...
        {aba === "automacoes" ? (
          <PainelAutomacoes sessaoId={sessaoId} sessoes={sessoes} />
        ) : aba === "canais" ? (
          <PainelConfig
            sessoes={sessoes}
